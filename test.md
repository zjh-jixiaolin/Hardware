...
`SSD` 的存储芯片类型（闪存存储技术）主要包括 `SLC`、`MLC`、`TLC` 和 `QLC` 等几种

-  `SLC`（单层存储单元）每个 `cell` **存储单元**只存储 **一个** 比特的数据，`SLC` 具有**高速**和**长寿命**的优点，但成本较高，一般多用于企业级高端产品，如**高端存储设备**、**嵌入式系统**和**工业控制**等。—— 理论擦写 `10` 万次以上。
- `MLC` （双层存储单元）每个 `cell` 存储单元可存储 **两个** 比特的数据，`MLC` 具有
