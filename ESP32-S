...
![image-20230601120514526](https://raw.githubusercontent.com/zjh-jixiaolin/map_strong/main/202306011213947.png)

电位器属于 **无极性（不分正负）** 器件，**可变电阻** 的一种，三个触点通过旋转旋钮改变 `2` 号触点的位置，从而改变阻值的大小，`1` 脚和 `3` 脚分别接开发板的 `3.3V` 和 `GND`，`2` 脚接模拟输入引脚。
...
```c
# 通过以下函数用于电位器实现
analogReadResolution(12); // ESP32默认设置分别率为12位（0-4095,即2的12次方）
analogRead(PIN_ADC); // 读取ADC值（模拟信号 ——> 数字信号）
analogReadMilliVolts(PIN_ADC); // 读取电压值
--------------------------------------------------
#include <Arduino.h>  
...
void loop() {
    int analogValue = analogRead(PIN_ADC); // 读取模拟信号（将模拟信号转换为数字信号）
    Serial.printf("ADC = %d\n",analogValue);
    delay(1000);
}
